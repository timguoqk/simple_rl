...
def _draw_state(screen,
    ...
    for i, p in enumerate(objects["passenger"]):
        ...
        top_left_point = width_buffer + cell_width * (pass_x - 1) + in_taxi_size, height_buffer + cell_height * (
        taxi_oomdp.height - pass_y) + in_taxi_size
        pygame.draw.rect(screen, color_ls[-i - 1],
                         top_left_point + (cell_width - 2 * in_taxi_size, cell_height - 2 * in_taxi_size), 0)
    ...
    for i, p in enumerate(objects["passenger"]):
        ...
        top_left_point = int(width_buffer + cell_width * (dest_x - 1) + 25), int(
            height_buffer + cell_height * (taxi_oomdp.height - dest_y) + 25)
        # circle_center = int(top_left_point[0] + cell_width/2.0), int(top_left_point[1] + cell_height/2.0)
        dest_col = (
        max(color_ls[-i - 1][0] - 30, 0), max(color_ls[-i - 1][1] - 30, 0), max(color_ls[-i - 1][2] - 30, 0))
